...
class RpcController(XMLRPCController):
    ...
    def campaign_update(self, campaign_id):
        ...
        offers = self._get_offers_from_AdLoad(campaign_id)
        offers.extend(app_globals.adload_rpc.get_offers_adload_getmyad(campaign_id))

        app_globals.db.offer.remove({'campaignId': campaign_id}, safe=True)

        date = datetime.date.today()
        date = datetime.datetime(date.year, date.month, date.day, 0, 0)
        last_clicksUnique_impressions = app_globals.db.stats_overall_by_date.find_one({'date': (date  - datetime.timedelta(days=7))},{"impressions":1, "clicksUnique":1, '_id' : 0})

        ctr = 0.016
        if last_clicksUnique_impressions:
            ctr = (last_clicksUnique_impressions['clicksUnique'] / last_clicksUnique_impressions['impressions']) * 100000

        keywords_campaign = camp.keywords
        ...
        exactly_phrases_campaign = camp.exactly_phrases
        minus_words_campaign = camp.minus_words
        
        for x in offers:
            offer_cost = 0.5
            offer = Offer(str(x['id']))
            offer.title = x['title']
            offer.price = x['price']
            offer.url = x['url']
            ...
            offer.listAds = ['ALL']
            offer.keywords = keywords_campaign
            offer.exactly_phrases = exactly_phrases_campaign
            offer.minus_words = minus_words_campaign
            offer.type = 'teaser'
            offer.uniqueHits = -1
            offer.width = -1
            offer.height = -1
            offer.rating = round((ctr * offer_cost), 4)
    ...
    def _get_offers_from_AdLoad(self, campaign_id):
        page = 0
        offers = []
        end = True
        while end:
            r = app_globals.adload_rpc.get_offers.page.rynok.getmyad(campaign_id, page)
