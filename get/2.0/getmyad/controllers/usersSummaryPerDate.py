...
class UserssummaryperdateController(BaseController):
    ...
    def userActivity(self, user_login, date):
        ...
        dateEnd = today
        data = model.StatisticReport().statGroupedByDate(advertises, dateStart=before_yesterday, dateEnd=today)
        if not data:
            return 'orangeflag'
    ...
    def active_domains(self, date):
        ...
        domain_data = {}
        for x in app_globals.db.stats_daily_adv.find({'date': date}):
