...
where a.id > 230000000 and dateRequest >= '@dateStart' and dateRequest < '@dateEnd' and [Action] = 1
order by a.id
...
from [1gb_YottosStat].dbo.StatisticAds a
inner join @lot lot on lot.id = a.LotID 
where a.id > 230000000 and dateRequest >= '@dateStart' and dateRequest < '@dateEnd' and [Action] = 0
    """.replace('@dateStart', formatDate(start)).replace('@dateEnd', formatDate(end))
    )
