...
#        all_accounts = [x['login'] for x in app_globals.db.users.find().sort('login')]
        ...
        adv = {}
        for advertise in app_globals.db.informer.find({'user': {'$in': all_accounts}}).sort('user').sort('domain').sort('title'):
    ...
    def structuredShowCondition(self, campaign_id):
        ...
        for allowed_adv in showCondition.allowed_informers:
            adv = app_globals.db.informer.find_one({'guid': allowed_adv})
            account = adv['user']
            domain = adv['domain']
            title = adv['title']
            guid = adv['guid']
        ...
        for ignored_adv in showCondition.ignored_informers:
            adv = app_globals.db.informer.find_one({'guid': ignored_adv})
            account = adv['user']
            domain = adv['domain']
            title = adv['title']
            guid = adv['guid']
