    ...
    def accountIncome(self):
        ...
        data = db.stats_daily_adv.group(['date'],
                                        ...
                                        '''function(o,p) {
                                          ...
                                          p.ban_unique += o.banner_clicksUnique || 0;
                                          p.teaser_sum += o.teaser_totalCost || 0;
                                          p.banner_sum += o.banner_totalCost || 0;
                                          }
                                          else{
                                          p.banner_impressions += o.banner_impressions || 0;
                                          p.imp_banner_sum += o.banner_totalCost || o.totalCost || 0;
                                          }
                                        }''')
        ...
        data = [{'id': index,
                 'cell': (
                    ...
                    x['unique'],
                    '%.2f $' % (x['teaser_sum'] / x['unique']) if x['unique'] else 0,
                    '%.2f $' % x['teaser_sum'],
                    x['banner_impressions'],
                    '%.2f $' % (x['imp_banner_sum'] / (x['banner_impressions']/1000)) if x['banner_impressions'] else 0,
                    '%.2f $' % x['imp_banner_sum'],
                    x['ban_unique'],
                    '%.2f $' % x['banner_sum'],
                    '%.2f $' % x['sum']
