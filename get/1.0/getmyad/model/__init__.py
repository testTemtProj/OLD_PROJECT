...
def setFixedCostForClick(user_login, cost, date):
    ...
    log.info(u'Установили %s пользователю %s фиксированную цену за клик %s', date, user_login, cost )
...
def setFloatingCostForClick(user_login, date, percent, cost_min, cost_max):
    ...
    log.info(u'Установили %s пользователю %s плаваюшию цену за клик (процент %s, минимум %s, максимум %s) ', date, user_login, percent, cost_min, cost_max)
