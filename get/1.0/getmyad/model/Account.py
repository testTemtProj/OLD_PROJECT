...
from getmyad.model import StatisticReports, mq
...
class ManagerReports():
    ...
    def monthProfitPerDate(self):
        ...
        advs = [x['guid'] for x in self.db.informer.find({'user' : {'$in': users}})]
        ...
        data = {}
        for adv in advs:
            for x in StatisticReport().statGroupedByDate(adv, dateStart, dateEnd):
