...
class RegisterAdvController(BaseController):
    ...
    def createUser(self):
        ...
        if c.account_type == 'user':
            ...
            try:
                ...
                c.click_cost = form_result['click_cost']
                c.percent = form_result['percent']
                c.cost_min = form_result['cost_min']
                c.cost_max = form_result['cost_max']
                if request.params.get('click_cost_type') == 'floating':
                    c.click_cost_type = 'floating'
                else:
                    c.click_cost_type = 'fixed'
                ...
                c.minsum = request.params.get('minsum')
                c.click_cost_type = request.params.get('click_cost_type')
                c.click_cost = request.params.get('click_cost')
                c.percent = request.params.get('percent')
                c.cost_min = request.params.get('cost_min')
                c.cost_max = request.params.get('cost_max')
                c.manager_get = request.params.get('manager_get')
                c.money_card = False if request.params.get('money_card') == None else True
                c.money_web_z = False if request.params.get('money_web_z')  == None else True
                c.money_factura = False if request.params.get('money_factura') == None else True
    ...
    def saveUser(self):
        ...
        try:
            ...
            log.info('Save new user account')
            account.register()
            account.domains.add(account.login)
            # Назначаем цену за клик
            if c.click_cost_type == 'floating':
                model.setFloatingCostForClick(c.siteUrl, datetime.now(),
                                              c.percent, c.cost_min,
                                              c.cost_max)
            else:
                model.setFixedCostForClick(c.siteUrl, float(c.click_cost),
                                           datetime.now())
        except:
            return False
        return True
...
class RegisterUserForm(Schema):
    ...
    click_cost = formencode.validators.Number(
                    not_empty=True,
                    messages={'empty': u'Введите цену за клик',
                              'number': u'Некорректная цена за клик'})
    percent = formencode.validators.Number(
                    not_empty=True,
                    messages={'empty': u'Введите процент от цены рекламодателя',
                              'number': u'Некорректный формат числа'})
    cost_min = formencode.validators.Number(
                    not_empty=True,
                    messages={'empty': u'Введите минимальную цену за клик',
                              'number': u'Некорректный формат числа'})
    cost_max = formencode.validators.Number(
                    not_empty=True,
