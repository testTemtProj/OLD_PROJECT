...
def jgridDataWrapper(data, userdata=None, page=None, records_on_page=None,
                     json=True):
    """ Принимает список значений колонок и возвращает json для таблиц jqGrid.
        
        Для организации пейджинга можно передать параметры ``page`` -- номер
        текущей страницы, и ``records_on_page`` -- количество записей на одной
        странице.
    """
    records = len(data)
