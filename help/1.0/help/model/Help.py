    ...
    def returnOne(self, date = None, id = None):
        ...
        if (id == None ):
            if isinstance(date, datetime.datetime):
                q = 'select * from news where project_id = "%s" and create_date = "%s" and lang_id = "%s" LIMIT 0 , 1'%(self.projectId, date.strftime("%Y-%m-%d %H:%M:%S"), self.langId)
            else:
                q = 'select * from news where project_id = "%s" and lang_id = "%s"  order by create_date desc LIMIT 0 , 1'%(self.projectId, self.langId)
        else:
            q = 'select * from news where project_id = "%s" and lang_id = "%s" and id= "%s" LIMIT 0 , 1'%(self.projectId, self.langId, id)
        query.query(q)
        count = 0
