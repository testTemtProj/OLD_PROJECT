...
protected:
	...
	string directoryName;///< имя папки, в которой расположен индекс. Вбито в код: "/var/www/index".
	char ch;///< символ '"', для удобства написания кода.
	char exCh;///< этим символом начинается и заканчивается каждая точная фраза.
public:
	SearcherWrapper()
	{
		//на самом деле надо как-то получать имя директории, тем более что надо прописать механизм "подмены" индекса
		//можно просто в файловой системе менять название папки? но потом же всё равно надо будет перегружать... %)
		directoryName = "/var/www/index";
		//LOG(INFO) << "папка, в которой расположен индекс: " << directoryName << "\n";
		searcher = new XXXSearcher(directoryName);
...
protected:
	...
	string getPriorityBannersString1(const string& informerId, const string& topics)
	{
		...
		string str = "+informer_ids:(all OR " + informerId + ") +abcde +exactly_phrases:(\"abcde\") +minus_words:(abcde) +type:banner +topic_ids:(" + topics + ")";
		//string str = "informer_ids:all AND keywords:abcde";
		//string str = "type:banner AND informer_ids:(" + informerId + ") AND keywords:abcde AND exactly_phrases:(\"abcde\") AND minus_words:(abcde) ";
...
public:
	/**
	 * Обращение к индексу CLucene. Этот метод не производит непосредственное обращение, он только формирует необходимый запрос и отдаёт его на выполнению классу XXXSearcher.
	 ...
	 * Идентификаторы - это идентификаторы РП, отобранных в результате обращения к индексу, веса - это веса, которые возвращает CLucene - веса соответствий РП запросу к индексу.
	 ...
	 */
	list< pair<string, float>  > f1(const string& query, UserHistory& userHistory, const vector<string>& context, const string& informerId, const list<Campaign>& campaigns)
	{
		...
		string deprStr = listOfStringsToString(userHistory.getDeprecatedOffers());
		...
		list< pair<string, float>  > l1 = searcher->process1(constructQuery(q, strCamp, deprStr)/*, A*/);
		...
		// +campaign_id:(все значения из списка кампаний)
		// -guid:(deprecatedOffers)
		LOG (INFO) << "2. запрос по query\n";
		q = getQueryString(query,informerId);
		list< pair<string, float>  > l2 = searcher->process1(constructQuery(q, strCamp, deprStr)/*, B*/);

		//3. РП по shortHistory - тот же запрос, что и в 2, только вместо query идёт КАЖДАЯ СТРОКА shortHistory
		LOG (INFO) << "3. запрос по shortHistory\n";
		...
		for (i=userHistory.getShortHistory().begin(); i!=userHistory.getShortHistory().end(); i++)
		{
			...
			list< pair<string, float>  > l = searcher->process1(constructQuery(q, strCamp, deprStr)/*, C*/);
		...
		// +campaign_id:(все значения из списка кампаний)
		// -guid:(deprecatedOffers)
		LOG (INFO) << "4. запрос по longHistory\n";
		...
		list< pair<string, float>  > l4 = searcher->process1(constructQuery(q, strCamp, deprStr)/*, D*/);
		
		//5. контекст. пока что нет.
		...
		list< pair<string, float>  > l6 = searcher->process1(constructQuery(q, strCamp, deprStr)/*, F*/);

		//теперь всё сливаем в результирующий список
	...
	list< pair<string, float>  > f1(const string& query, UserHistory& userHistory, const vector<string>& context, const string& informerId, const list<Campaign>& campaigns, const Weights &w)
	{
		...
		list< pair<string, float>  > l1 = searcher->process1(constructQuery(q, strCamp, deprStr), w.range_priority_banners);
		...
		q = getQueryString(query,informerId);
		list< pair<string, float>  > l2 = searcher->process1(constructQuery(q, strCamp, deprStr), w.range_query);
		...
		for (i=userHistory.getShortHistory().begin(); i!=userHistory.getShortHistory().end(); i++)
		{
			...
			//if(!str.empty())
			//	deprStr += (" " + str);//deprecatedOffers результат2
			l = searcher->process1(constructQuery(q, strCamp, deprStr), w.range_shot_term);
		...
		////if(!str.empty())
		////	deprStr += (" " + str);//deprecatedOffers результат2 результат3
		list< pair<string, float>  > l4 = searcher->process1(constructQuery(q, strCamp, deprStr), w.range_long_term);
		...
		////if(!str.empty())
		////	deprStr += (" " + str);//deprecatedOffers результат1 результат2 результат3 результат4
		list< pair<string, float>  > l6 = searcher->process1(constructQuery(q, strCamp, deprStr), w.range_on_places);
		LOG(INFO) << "query -> "<<q;
