...
/** \mainpage KeywordsIndexCreator service
    ...
    \section usage_section »спользование
	...
	ћодуль €вл€етс€ фоновым приложением, запускаемым как сервис из /etc/init.d

    ѕримеры использовани€ из командной строки (indexator.conf - файл конфигурации, см. ниже):
    ...
    \section settings_section Ќастройка сервиса

    keywords_indexcreator дл€ построени€ индекса использует базу данных mongo, 
	котора€ содержит описание всех рекламных предложений. Ёто основна€ база данных проекта,
       которую использует пользовательска€ часть. Ќа уровне приложени€
       регистрируетс€ как база данных по умолчанию (см. DB::addDatabase).


    ѕараметры подключени€ к базе данных хран€тс€ в файле конфигурации.
	ѕараметры \c mongo_main_host, \c mongo_main_db содержат адрес (в формате
    "host[:port]" и названи€ баз данных. 

    ѕо умолчанию будут использованы следующие параметры:
	...
	 роме того, в файле конфигурации хранитс€ переменна€ \c index_folder, отвечающа€ за 
	место размещени€ индекса на физическом диске.
	...
	ѕример конфигурационного файла:
...
/**
‘ункци€ создани€ файла с PID'ом процесса
*/
void SetPidFile(const char* Filename, pid_t pid)
{
    FILE* f;
