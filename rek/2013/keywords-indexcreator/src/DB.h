...
public:
	...
	/** Вспомогательный метод, возвращающий значение поля field как bool.
        Если ни одно преобразование не сработало, вернёт False.
     */
    static bool toBool(const BSONElement &element) {
        switch (element.type()) {
        case Bool:
            return (bool)element.boolean();
        case NumberInt:
            return (element.numberInt() != 0);
		case NumberDouble:
			return (((float)element.numberDouble()) != 0.0f);
        case String:
            try {
                return boost::lexical_cast<bool>(element.str());
            } catch (boost::bad_lexical_cast &) {
                return false;
            }
        default:
