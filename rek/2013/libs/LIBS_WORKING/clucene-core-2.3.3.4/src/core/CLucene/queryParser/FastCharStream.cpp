...
void FastCharStream::refill() {
	...
	if (tokenStart == 0) {			  // token won't fit in buffer
		if (buffer == NULL) {		  // first time: alloc buffer
			buffer = _CL_NEWARRAY(TCHAR, 20000); //TODO: А размер буферов надо сделать динамическим
			_bufferSize = 20000;                 // чтоб он создавался от количества символов в запросе а не фексированно
