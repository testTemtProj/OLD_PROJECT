...
void IndexCreatorService::InitDefault()
  {
      ...
      stop_words_="/etc/keywords-indexcreator/stopwords.conf";
    }
...
void IndexCreatorService::InitDBs(std::string fileName)
{
	...
	LOG(INFO)<<"Connection params in file:" << fileName;
        ...
        if(!str.empty())                     // Ignore empty lines
           {
              ...
              else if (paramName.compare("stop_words")==0) stop_words_=paramValue;
...
int IndexCreatorService::Serve()
{
    if (!ConnectDatabase()) {
		...
		core = new Core(index_folder_offer_, index_folder_informer_, stop_words_);
    	return core->Process();
    }
