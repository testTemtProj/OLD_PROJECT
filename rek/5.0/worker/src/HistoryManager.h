...
public:
	...
	*/	
	...
	{
		searcher.setLuceneIndexParams(folder_offer, folder_informer);
	}
...
/** \brief  Инициализация весов для задания приоритететов групп рекламных предложений при формировании списка предложений к показу 
	...
	*/	
	 ...
	 */
	list<pair<pair<string, float>, pair<string, string>>> getOffersByUser1(const Params& params, list<Campaign>& campaigns, list<Campaign>& campaignsAllGeo)
	{
		...
		list<pair<pair<string, float>, pair<string, string>>> result = searcher.f1(params.getUserQueryString(), *uh, params.getContext(), params.getInformer(), campaigns, campaignsAllGeo, w);
	...
	/** \brief Получение значения рейтингов РП для данного рекламного блока
	 *
	 * \param params - параметры запроса.
	 * \param offersIds - список  РП, для которых нужно извлечь из БД их рейтинг для текущего рекламного блока
	 */
	map< string, float > getOffersRatingByInformer(const Params& params, list<pair<pair<string, float>, pair<string, string >>>& offersIds)
	{
		
		map< string, float > result = searcher.ratingSearch(params.getInformer(), offersIds);
		return result;
	...
	*/
	bool updateUserHistory(const vector<Offer>& offers, const Params& params, bool clean, bool updateShort, bool updateContext)
	{
        ...
        if (updateShort)
        {
		    ...
		    accessor.updateShortHistory(params.getUserKey(), params.getUserQueryString());
        }
        if (updateContext)
        {
