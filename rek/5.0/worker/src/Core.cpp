...
std::string Core::Process(const Params &params)
{
	...
	list<pair<pair<string, float>, pair<string, string>>> offersIds;
	...
	if(!camps.empty()){
        ...
        //Запрос к индексу на получение рейтингов по РП для конкретному рекламному блока полученых в редыдушем запросе
		offersRatingIds = HistoryManager::instance()->getOffersRatingByInformer(params, offersIds);
	}
	else{
	...
	//{
	//	VLOG(2) << (*i).first.first << " " << (*i).first.second << " " << (*i).second.first << " " << (*i).second.second;
	//}


	//если полученный вектор пуст, используем старый алгоритм без контекста и банеров, в противном случае используем новый алгоритм
	if (offersIds.size()==0)
	{
	...
	{
		//новый алгоритм
		offers = getOffersRIS(offersIds, offersRatingIds, params, camps, clean, updateShort, updateContext);
		if (!offers.size())
		{
...
vector<Offer> Core::getOffersRIS(const list<pair<pair<string, float>, pair<string, string>>> &offersIds, map< string,float > &offersRatingIds, const Params &params, const list<Campaign> &camps, bool &clean, bool &updateShort, bool &updateContext)
{
	vector<Offer> result;
	createVectorOffersByIds(offersIds, offersRatingIds, result, camps, params, updateShort, updateContext);
 ...
 * 
 * Если список offersIds пуст, то метод в качестве результата выберет все РП из всех РК, находящихся в списке camps,
 * и отсортирует результат по убыванию рейтинга.
 * Поэтому в методе Process если список offersIds пуст, вызывается старая ветка.
 */
void Core::createVectorOffersByIds(const list<pair<pair<string, float>, pair<string, string>>> &offersIds, map< string,float > &offersRatingIds, vector<Offer> &result, const list<Campaign> &camps, const Params& params, bool &updateShort, bool &updateContext)
{   
	map< string,float >::iterator it;
	Informer informer(params.getInformer());
    list<pair<pair<string, float>, pair<string, string>>>::const_iterator p = offersIds.begin();
    float curScore = offersIds.begin()->first.second;
    ...
    while(p!=offersIds.end())
    {
        ...
        curOffer->setNewBranch(p->second.first);
        curOffer->setNewConformity(p->second.second);
        ...
        if (curOffer->valid() && checkBannerSize(*curOffer, informer))
        {
            ...
            if (curScore!=p->first.second)
            {
                //вышли за пределы одного score, надо отсортировать офферы по рейтингу в теперь уже предыдущем score
                end = result.end();
                //end показывает на элемент, который только что добавили
                end--;
                start = result.begin() + xstart;
                //отсортировали
                std::sort(start, end, inverseOrderCompare);
                //теперь задаём начало нового (текущего) score
                //теперь и start показывает на элемент, который только что добавили
                start = end;
                xstart += lenght;
                curScore=p->first.second;
                lenght = 0;
            }
            else
            {
