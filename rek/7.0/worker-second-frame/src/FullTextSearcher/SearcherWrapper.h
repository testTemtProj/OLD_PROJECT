...
public:
	 ...
	 */
	list< pair< pair<string, float>, pair<string, pair<string, string>> > > f1(const string& query, UserHistory& userHistory, const string& context, const string& informerId, const list<Campaign>& campaigns, const list<Campaign>& campaignsAllGeo, const Weights &w)
	{
		...
		map<string, pair<pair<float,pair<pair<string,string>,pair<string,string>>>,pair<string,string>>> MAPSEARCH;
        searcher->processOffer(q, strCamp, categoryStr, deprStr, MAPSEARCH);
        ...
        if (!strSS.empty())
        {
            ...
            if (!q.empty())
            {
                stringQuery.push_back(pair<string,pair<float,string>>(q, pair<float,string>(0.1,"T1")));
            }
        }
        ...
        */
        ...
        for (i=userHistory.getShortHistory().begin(); i!=userHistory.getShortHistory().end(); i++)
        {
            ...
            if (!strSH.empty())
            {
                ...
                if (!q.empty())
                {
                    stringQuery.push_back(pair<string,pair<float,string>>(q, pair<float,string>(0.1,"T3")));
                }
            }
        }
        /*
        ...
        */
        ...
        for (itr=userHistory.getLongHistory().begin(); itr!=userHistory.getLongHistory().end(); itr++)
        {
            ...
            {   
                ...
                if (!q.empty())
                {
                    stringQuery.push_back(pair<string,pair<float,string>>(q, pair<float,string>(0.1,"T5")));
                }
            }
        }
            ...
            if (p->second.first.second.first.second == "false")
            {
                if(p->second.first.second.second.first == "banner" and p->second.first.second.second.second == "false")
                {
                    resultImpression.push_back(pair<pair<string, float>,pair<string, pair<string, string>>>(
                                ...
                                (pair<string, pair<string, string>>(p->second.first.second.first.first,(pair<string, string>(p->second.second.first,p->second.second.second))))));
                }
                else
