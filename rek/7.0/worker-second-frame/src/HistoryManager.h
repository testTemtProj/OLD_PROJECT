...
/** \brief  Инициализация весов для задания приоритететов групп рекламных предложений при формировании списка предложений к показу 
	...
	*/	
	 ...
	 */
	list<pair<pair<string, float>, pair<string, pair<string, string>>>> getOffersByUser1(const Params& params, list<Campaign>& campaigns, list<Campaign>& campaignsAllGeo)
	{
		...
		list<pair<pair<string, float>, pair<string, pair<string, string>>>> result = searcher.f1(params.getUserQueryString(), *uh, params.getContext(), params.getInformer(), campaigns, campaignsAllGeo, w);
