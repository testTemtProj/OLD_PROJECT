...
string UrlParser::stringToUtf8(string &str){
	...
	//	{
			...
			//в цикле проходим по строке. если встречаем '%', значит за ним должны быть ещё 2 символа, обозначающие в кодировке windows-1251 один байт.
            //этот байт нам и нужно заменить на соответствующий ему байт в кодировке utf-8
			size_t lookHere = 0;
			size_t foundHere;
			string from, to;
			while((foundHere = str.find("%", lookHere)) != string::npos)
