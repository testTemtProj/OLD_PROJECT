...
{
	//LOG(INFO) << "Core конструктор: перед загрузкой.\n";
    LoadAllEntities();
	//LOG(INFO) << "Core конструктор: после загрузки.\n";
    InitMessageQueue();
    InitMongoDB();
...
{
	//LOG(INFO) << "\n\n\nreferrer = " << params.referrer_ << ", userQueryString=" << params.userQueryString_;
	//Benchmark bench("\nвремя обработки запроса (время работы Core::Process) ");
...
/*
	...
	int k = 0;
  	for (list<Campaign>::iterator it=camps.begin(); it!=camps.end(); ++it)
	{
		k++;
		//if (k==1) continue;
    		result.push_back(*it);
...
///////////////////////////////////////////////////////////
	...
	//offers.push_back(Offer("464fa8fd-1d3c-43e4-80e7-e2c38a911b6b"));//gif
	//offers.push_back(Offer("464fa8fd-1d3c-43e4-80e7-e2c38a911b6c"));//flash
	//LOG(INFO) << "informer: w=" << informer.width() << ",  h=" << informer.height() << "\n\n";

	//LOG(INFO) << "РП для отображения отобраны, offers.size()=" << offers.size() << "\n";

	/*for(size_t i=0; i<offers.size(); i++)
	{
		LOG(INFO) << "id=" << offers[i].id() << "\n";
		LOG(INFO) << "title=" << offers[i].title() << "\n";
		LOG(INFO) << "price=" << offers[i].price() << "\n";
		LOG(INFO) << "description=" << offers[i].description() << "\n";
		LOG(INFO) << "url=" << offers[i].url() << "\n";
		LOG(INFO) << "image_url=" << offers[i].image_url() << "\n";
		LOG(INFO) << "campaign_id=" << offers[i].campaign_id() << "\n";
		LOG(INFO) << "valid=" << offers[i].valid() << "\n";
		LOG(INFO) << "rating=" << offers[i].rating() << "\n";
		LOG(INFO) << "uniqueHits=" << offers[i].uniqueHits() << "\n";
		LOG(INFO) << "height=" << offers[i].height() << "\n";
		LOG(INFO) << "width=" << offers[i].width() << "\n";
		LOG(INFO) << "type=" << offers[i].type() << "\n";
		LOG(INFO) << "\n\n";
	}*/
		       ...
		       const Params &params, list<string> &shortTerm, list<string> &longTerm )
{
	...
	LOG(INFO) << "log.impr -> id = "<<i.offer.id();
	LOG(INFO) << "log.impr -> title = "<<i.offer.title();
	LOG(INFO) << "log.impr -> campaign = "<<i.offer.campaign_id();
