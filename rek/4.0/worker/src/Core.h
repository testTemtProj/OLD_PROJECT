...
public:
     ...
     */
    std::vector<Offer> getOffers(int count, const Params &params);
     ...
     *
     * Возвращает РП, отобранные для показа по новому алгоритму RISAlgorithm.
     * 
     ...
     * 
	 * Вход: список идентификаторов допустимых РП, параметры Params, список кампаний, по которым отбирались допустимые РП.\n
	 * 1. По входному списку идентификаторов создаётся массив РП. Одновременно, при создании, происходит фильтрация для баннеров и сортировка в рамках одного веса. Приоритет веса больше, чем приоритет рейтинга. Подробнее - см. комментарии к методу createVectorOffersByIds.\n
	 * 2. Выбор необходимого для данного информера количества РП по новому алгоритму. В результате работы нового алгоритма отображаются РП с наибольшим весом, а в рамках одного веса - с наибольшим рейтингом.
	 * \see RISAlgorithm
	 * \see createVectorOffersByIds
     */
	std::vector<Offer> getOffersRIS(const list< pair<string,float> > &offersIds, const Params &params, const list<Campaign> &camps);
	 ...
	 * \see createVectorOffersByIds
     */
	std::vector<Offer> getOffersRIS(const list< pair<string,float> > &offersIds, map< string,float > &offersRatingIds, const Params &params, const list<Campaign> &camps);
...
private:
	...
	void createVectorOffersByIds(const list< pair<string,float> > &offersIds, vector<Offer> &result, const list<Campaign> &camps, const Params& params);
	void createVectorOffersByIds(const list< pair<string,float> > &offersIds, map< string,float > &offersRatingIds, vector<Offer> &result, const list<Campaign> &camps, const Params& params);
	...
	void RISAlgorithm(vector<Offer> &result, const Params &params);
