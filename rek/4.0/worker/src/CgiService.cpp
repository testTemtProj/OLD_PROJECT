    ...
    : argc(argc), argv(argv), core(0)
{
	...
	redis_short_term_history_port_ = getenv("redis_short_term_history_port", "6379");
	redis_long_term_history_host_ = getenv("redis_long_term_history_host", "127.0.0.1");
	redis_long_term_history_port_ = getenv("redis_long_term_history_port", "6380");
	...
	redis_page_keywords_port_ = getenv("redis_page_keywords_port", "6382");
	...
	range_context_ = atof(getenv("range_context", "0.25").c_str());
...
void CgiService::Serve()
{
	...
	RISinit();//добавлено RealInvest Soft
                ...
                const std::string visitor_cookie)
{
	...
	try {
	    string result = core->Process(Params()
					...
					.w(url.param("w"))
					.h(url.param("h"))
					.userQueryString(url.param("userQueryString"))
					 );
	...
	Response("", 503);
	    ...
	    LOG(INFO) << "exception " << typeid(ex).name() << ": " << ex.what()
		       << " while processing " << query;
...
/** Добавлено RealInvest Soft. Все необходимые действия для инициализации модуля вынесены в отдельный метод. */
void CgiService::RISinit()
{
	...
	{
		LOG(INFO) << "Хранилище соответствий поисковиков значениям параметров не проинициализировано. Строка запроса с поисковика, которую вбил пользователь, разбираться не будет.\n";
	...
	{
		LOG(INFO) << "Сбой при инициализации шаблонов.\n";
