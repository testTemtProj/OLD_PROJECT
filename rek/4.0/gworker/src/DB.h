...
public:
    ...
    /** \brief  Описывает строку подключения к MongoDB Replica Set. */
    class ReplicaSetConnection
    {
	std::string replica_set_;
	std::string connection_string_;
    public:
	/** \brief  Создаёт строку подключения к MongoDB Replica Set.

        \param replica_set        название replica_set
	    \param connection_string  строка подключения вида
    ...
    */
    ...
    /** \brief  Регистрирует подключение \a name к базе данных.
    ...
    /** \brief  Регистрирует подключение \a name к набору реплик баз данных
	(Replica Set). */
    static void addDatabase(const std::string &name,
    ...
    /** \brief  Регистрирует базу данных "по умолчанию". */
    ...
    /** \brief  Регистрирует базу данных "по умолчанию", подключение
     *          осуществляется к набору реплик (Replica Set). */
    ...
    /** \brief  Возвращает полное название коллекции */
    std::string collection(const std::string &collection) {
	return database() +  "." + collection;
    }

    /** \brief  Название используемой базы данных */
    std::string &database() {
	return options_->database;
    }

    /** \brief  Адрес сервера MongoDB */
    std::string &server_host() {
	return options_->server_host;
    }

    /** \brief  Название набора реплик (replica set) */
    std::string &replica_set() {
	return options_->replica_set;
    }

    /** \brief  Возвращает true, если допускается read-only подключение к
     *          slave серверу в кластере */
    bool slave_ok() {
    	return options_->slave_ok;
    }

    /** \brief  Возвращает соединение к базе данных.
     *
    ...
    /** \brief  Вспомогательный метод, возвращающий значение поля field как int.
     *
    ...
    /// \name Обёртки над DBClientConnection
    ///
    ...
    /// параметр \c coll (collection) и автоматически добавляющие к названию
    /// коллекции имя базы данных.
    ///
    /// Некоторые методы (insert(), update(), remove()) принимают также
    /// дополнительный параметр \c safe, который, если равен true, заставляет
    ...
    /** \brief  Исключение, возникающее при попытке обратиться к незарегистированной
	            базе данных.
    */
...
protected:
    ...
    struct ConnectionOptions {
        ...
        /// Название реплики MongoDB (если монго запущена в репликах).
        std::string replica_set;

        /// Может ли подключаться к slave узлам базы данных.
        bool slave_ok;
    } *options_;
