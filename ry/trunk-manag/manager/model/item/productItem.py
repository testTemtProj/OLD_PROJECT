...
class ProductItem():
    ...
    def save(self):
        base_model = Base()
        product_collection = base_model.get_product_collection()

        if vars(self).has_key("LotID"):
            self._id = product_collection.save(self.to_array())
        else:
            last = product_collection.find().sort('LotID', desc)[0]
            id = int(last["LotID"]) + 1
            self.LotID = id
            self._id = product_collection.save(self.to_array())
