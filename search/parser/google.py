...
class GoogleSearch(object):
    SEARCH_URL = "http://www.google.%(tld)s/search?hl=%(lang)s&q=%(query)s&num=%(num)d&btnG=Google+Search"
    NEXT_PAGE = "http://www.google.%(tld)s/search?hl=%(lang)s&q=%(query)s&num=%(num)d&start=%(start)d"

    def __init__(self, query, random_agent=False, debug=False, lang="ru", tld="ru", re_search_strings=None):
        self.query = query
        self.debug = debug
    ...
    def _get_results_page(self):
        if self._page == 0:
            url = GoogleSearch.SEARCH_URL
        else:
            url = GoogleSearch.NEXT_PAGE
